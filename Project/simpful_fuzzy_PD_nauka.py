...
# Predefiniowane prawe boki dla uchybu (e)
prawe_boki_e = [-4, -3, -2, -1, 1, 2, 3, 4, 5.1]
wszystkie_etykiety = ["BDU", "DU", "ŚU", "MU", "Z", "MD", "ŚD", "DD", "BDD"]
#
# Inicjalizacja fuzzy setów dla 'e'
fuzzy_sets_e = []
lewy = -5.1  # pierwszy lewy bok – stały

for prawy, label in zip(prawe_boki_e, wszystkie_etykiety):
    srodek = (lewy + prawy) / 2
    fuzzy_sets_e.append(
        FuzzySet(
            function=Triangular_MF(lewy, srodek, prawy),
            term=label
        )
    )
    lewy = srodek  # lewy bok kolejnego trójkąta to środek bieżącego

# Dodanie zmiennej lingwistycznej dla 'e'
...
# To jest to samo co kod zakomentowany niżej
# Predefiniowane prawe boki dla pochodnej uchybu (de)
prawe_boki_de = [-1, -0.75, -0.5, -0.25, 0.1, 0.5, 0.75, 1, 1.2]
# wszystkie_etykiety = ["BDU", "DU", "ŚU", "MU", "Z", "MD", "ŚD", "DD", "BDD"]

# Inicjalizacja fuzzy setów dla 'de'
fuzzy_sets_de = []
lewy = -1.2  # pierwszy lewy bok – stały

for prawy, label in zip(prawe_boki_de, wszystkie_etykiety):
    srodek = (lewy + prawy) / 2
    fuzzy_sets_de.append(
        FuzzySet(
            function=Triangular_MF(lewy, srodek, prawy),
            term=label
        )
...
# Dodanie zmiennej lingwistycznej dla 'de'
...
# To jest to samo co kod zakomentowany niżej
# Predefiniowane prawe boki trójkątów
prawe_boki = [-80, -60, -40, 0, 10, 40, 80, 100]
prawe_boki.append(120)
# wszystkie_etykiety = ["BDU", "DU", "ŚU", "MU", "Z", "MD", "ŚD", "DD", "BDD"]
#
# # Inicjalizacja
fuzzy_sets = []
lewy = -120  # pierwszy lewy bok – stały

for prawy, label in zip(prawe_boki, wszystkie_etykiety):
    srodek = (lewy + prawy) / 2
    fuzzy_sets.append(
        FuzzySet(
            function=Triangular_MF(lewy, srodek, prawy),
...
# Dodanie zmiennej lingwistycznej
...
wartosc_na_etykiete = {i - 4: ety for i, ety in enumerate(wszystkie_etykiety)}
def liczby_na_etykiety(tabela_num):
    return [
        [wartosc_na_etykiete[wartosc] for wartosc in wiersz]
        for wiersz in tabela_num
    ]
tabela_regul = liczby_na_etykiety(tabela_regul_num)
